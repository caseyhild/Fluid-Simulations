...
public class FluidVectorField extends JFrame implements Runnable, MouseListener, MouseMotionListener
{
    ...
    public FluidVectorField()
    {
        ...
        //make the fluid
        fluidCube = new FluidCube(0.0000001, 0.2);
    ...
    private void update()
    {
        ...
        {
            fluidCube.addDensity(mouseX * (fluidCube.size - 2)/width + 1, mouseY * (fluidCube.size - 2)/height + 1, 1);
            Vector2D vel = new Vector2D(mouseX - oldMouseX, mouseY - oldMouseY);
            fluidCube.addVelocity(mouseX * (fluidCube.size - 2)/width + 1, mouseY * (fluidCube.size - 2)/height + 1, vel.x, vel.y);
        }
    ...
    private void render()
    {
        ...
        for(int y = 0; y < height; y++)
        {
            ...
            {
                int x_grid = x * (fluidCube.size - 2)/width + 1;
                int y_grid = y * (fluidCube.size - 2)/height + 1;
                int shade = (int) (255 * fluidCube.density[y_grid][x_grid]);
        ...
        for(int y = 0; y < height; y++)
        {
            for(int x = 0; x < width; x++)
            {
                int x_grid = x * (fluidCube.size - 2)/width + 1;
                int y_grid = y * (fluidCube.size - 2)/height + 1;
                int boxSize = width/(fluidCube.size - 2);
                g.setColor(new Color(255, 0, 0));
                if(x % boxSize == 0 && y % boxSize == 0)
                {
                    g.drawLine(x + boxSize/2, y + boxSize/2, x + boxSize/2 + (int) (500 * fluidCube.velocity[y_grid][x_grid].x), y + boxSize/2 + (int) (500 * fluidCube.velocity[y_grid][x_grid].y));
                }
